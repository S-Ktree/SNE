...
OPTIONS1 = odeset('RelTol',1e-2);
OPTIONS2 = odeset('RelTol',1e-6);
sol=ode45(@double_pendulum_ODE,[0 duration], ivp, OPTIONS1);
sol2=ode45(@double_pendulum_ODE,[0 duration], ivp, OPTIONS2);
...
phi12=y2(1,:)'; dtphi12=y2(2,:)';
phi22=y2(3,:)'; dtphi22=y2(4,:)';
l1=ivp(8); l2=ivp(9);
% phi1=x(:,1); dtphi1=x(:,2);
% phi2=x(:,3); dtphi2=x(:,4);
% l1=ivp(8); l2=ivp(9);
...
set(gca,'nextplot','replacechildren');
    ...
    if movie==true
        %movie2avi(F,'doublePendulumAnimation.avi','fps',fps,'quality',50)
		movie2gif(F,'doublePendulumAnimation.gif','DelayTime',1/fps)
	end
...
set(gca,'nextplot','replacechildren');
    ...
    for i=1:length(phi1)-1
        if (ishandle(h)==1)
            Xcoord=[0,l1*sin(phi12(i)),l1*sin(phi12(i))+l2*sin(phi22(i))];
            Ycoord=[0,-l1*cos(phi12(i)),-l1*cos(phi12(i))-l2*cos(phi22(i))];
            set(h,'XData',Xcoord,'YData',Ycoord);
            drawnow;
            F(i) = getframe;
            if movie==false
    ...
    if movie==true
        %movie2avi(F,'doublePendulumAnimation.avi','fps',fps,'quality',50)
		movie2gif(F,'doublePendulumAnimation2.gif','DelayTime',1/fps)
    end