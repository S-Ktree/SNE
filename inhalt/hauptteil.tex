...
\begin{align}
	\Phi(x,t,T) &= x_{k} + T\, \sum\limits_{k=1}^s b_i \, k_i \quad \text{mit}\\
...
Eine weitere Klasse der Mehrschrittverfahren sind die so genannten \textit{Backwards Differentiation Formulas} (BDF). Für diese Verfahren gilt $P_b(z) = z^k$. Damit lassen sich Verfahren erzeugen, die bis zur Ordnung $p=k=6$ stabil sind.
