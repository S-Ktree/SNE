...
Dabei nimmt man für die Steigung im betrachteten Intervall den Mittelwert der Steigungen im $i$-ten und $i+1$-ten Schritt an:
\begin{align}
	x_{i+1} &= x_i + \Delta x_i = \\
			& = x_i + T \dd \eh \dd \Big(f\big(x_i,t_i\big) + f\big(x_{i+1},t_{i+1}\big)  \Big).
\end{align}
...
Da die Differentation der Rechten Seite jedoch nach mit verhältnismäßig großem rechnerischen Aufwand verbunden ist, sind diese Verfahren in der Praxis eher ungebräuchlich und sollen im Rahmen dieser Arbeit nicht näher betrachtet werden.

Für die betrachteten Integrationsverfahren lässt sich der \textit{globale} Fehler $e(t_i):= \norm{x_i - x(t_i)}$ durch
\begin{align}
	e(t) = \mathcal{O}(T^p)
\end{align}
abschätzen. (Getroffene Annahmen über die rechte Seite der Differentialgleichung (\ref{eq:AWA}) sollen im Rahmen dieser Arbeit außer Acht gelassen werden und können in \cite{gruene} nachgelesen werden.) Dabei legt die Konsistenzordnung $p$ fest, wie schnell der globale Fehler gegen Null geht, wenn die Schrittweite verkleinert wird. Es kann gezeigt werden, dass die Konsistenzordnung der Euler-Verfahren $p=1$ beträgt. Das Heun-Verfahren ist mit $p=2$ bereits deutlich genauer.\\
